"""Cost-safety watcher for the RivaQuant RunPod pod. Stops the pod (releases
the GPU, ends billing) the moment either the hard dollar cap is hit or
training reports done/error — mirrors the box_watcher.py pattern used for
the Vast.ai runs earlier tonight: never trust the training job to clean up
after itself, watch it from outside.
"""
import json
import os
import subprocess
    ...
    try:
        out = subprocess.run(
            ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=10",
             "-p", str(port), f"root@{ip}", "cat /workspace/rivaquant-out/status.json"],
...
def main() -> None:
    ...
    started = time.time()
    print(f"watching pod {pod_id} — cap ${COST_CAP_USD:.2f} at ${HOURLY_PRICE}/hr "
          f"(~{COST_CAP_USD / HOURLY_PRICE:.1f}h budget)")
